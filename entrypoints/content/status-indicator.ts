...
export function showPageStatus(color: string, text: string): void {
  ...
  dot.setAttribute('style', `display:inline-block;width:10px;height:10px;border-radius:50%;background:${escapeHtml(color)};box-shadow:0 0 6px ${escapeHtml(color)};flex-shrink:0;`)

  const textSpan = document.createElement('span')
  textSpan.setAttribute('style', 'color:#ccc;font-size:12px;')
