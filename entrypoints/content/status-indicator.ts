...
export function showPageStatus(color: string, text: string): void {
  ...
  dot.style.cssText = `display:inline-block;width:10px;height:10px;border-radius:50%;background:${color};box-shadow:0 0 6px ${color};flex-shrink:0;`

  const textNode = document.createElement('span')
  textNode.style.cssText = 'color:#ccc;font-size:12px'
  textNode.textContent = text

  el.replaceChildren(dot, textNode)
