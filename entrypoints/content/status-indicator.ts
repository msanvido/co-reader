...
export function showPageStatus(color: string, text: string): void {
  ...
  // 🛡️ Sentinel: Safe DOM manipulation without innerHTML to prevent DOM XSS
  const dotSpan = document.createElement('span')
  dotSpan.style.cssText = `display:inline-block;width:10px;height:10px;border-radius:50%;background:${color};box-shadow:0 0 6px ${color};flex-shrink:0;`

  const textSpan = document.createElement('span')
  textSpan.style.cssText = `color:#ccc;font-size:12px;`
  textSpan.textContent = text

  el.replaceChildren(dotSpan, textSpan)
